---
name: minions-portfolio-manager
id: OC-0100
version: 1.0.0
description: "Agent skills for working with Minions Portfolio-manager MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-portfolio-manager data."
category: ai
subcategory: general
...
# Minions Portfolio-manager Agent Skills

Skills for agents operating on the `minions-portfolio-manager` toolbox.
...
```bash
# TypeScript
pnpm add @minions-portfolio-manager/sdk

# Python
pip install minions-portfolio-manager

# CLI
pnpm add -g @minions-portfolio-manager/cli
```
...
The `portfolio-manager` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-portfolio-manager/sdk/schemas';
...
```python
from minions_portfolio_manager.schemas import custom_types
...
- This agent only writes to `minions-portfolio-manager` — it reads from other toolboxes but never writes to them